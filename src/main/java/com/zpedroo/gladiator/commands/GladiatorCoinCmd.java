...
public class GladiatorCoinCmd implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length < 3 || !isNumeric(args[2])) {
            sender.sendMessage(ChatColor.translateAlternateColorCodes('&', Main.get().getFiles().get("CONFIG").get().getString("Messages.gladiator-coin-usage")));
            return true;
        }
        ...
        if (args[0].toUpperCase().equals("GIVE")) {
            ...
            ItemStack item = ItemBuilder.build(Main.get().getFiles().get("CONFIG"), "TradeItem").build();
